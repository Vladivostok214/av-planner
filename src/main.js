...
const updateProject = async (projectId, newData) => {
    ...
    if (isSheets) {
        ...
        window.isEditing = false; setTimeout(loadData, 1000);
    } else {
...
window.formatScript = (cmd) => {
    if (cmd === 'scene') {
        ...
        const node = document.createElement('div'); node.className = 'script-scene'; node.innerHTML = 'INT. LUGAR - DIA';
        range.insertNode(node);
    } else if (cmd === 'dialogue') {
        const sel = window.getSelection(); const range = sel.getRangeAt(0);
        const node = document.createElement('div'); node.className = 'script-dialogue'; node.innerHTML = '<span class="character">PERSONAJE</span><br>Habla aqui...';
        range.insertNode(node);
    } else { document.execCommand(cmd, false, null); }
};

window.downloadPDF = () => {
    const doc = new jsPDF(); const p = window.appState.currentProject;
    const content = document.getElementById('scriptContent').innerText;
    doc.setFont("helvetica", "bold"); doc.setFontSize(22); doc.text(p.title.toUpperCase(), 20, 20);
    doc.setDrawColor(0, 58, 122); doc.line(20, 25, 190, 25);
    doc.setFont("courier", "normal"); doc.setFontSize(12);
    const splitText = doc.splitTextToSize(content, 170); doc.text(splitText, 20, 40);
    doc.save("GUION_" + p.title.replace(/\s+/g, '_') + ".pdf");
};
...
const renderApp = () => {
    ...
    if (!document.getElementById('script-styles')) {
        ...
        style.innerHTML = '.script-scene { font-weight: bold; text-transform: uppercase; margin-top: 2em; border-bottom: 1px solid #ddd; padding-bottom: 4px; } .script-dialogue { margin: 1em 20%; text-align: center; } .character { font-weight: bold; display: block; margin-bottom: 2px; }';
    ...
    if (window.appState.view === 'dashboard') {
        ...
        root.innerHTML = '<div class="p-6 md:p-10 max-w-[1300px] mx-auto min-h-screen"><header class="mb-10 flex flex-col md:flex-row justify-between items-end gap-6 border-b-2 border-brand-hairline pb-8"><div><h1 class="text-2xl font-bold text-brand-dark mb-1">PANEL DE PRODUCCION</h1><p class="text-[10px] font-bold text-brand-gray tracking-widest uppercase">Operador: <span class="text-brand-dark underline font-bold">' + window.appState.userName + '</span></p></div><div class="flex flex-wrap items-center gap-3"><div class="flex items-center gap-2 border-2 border-brand-hairline bg-white px-4 py-2 rounded-lg shadow-sm"><input type="text" id="searchInput" value="' + window.appState.searchQuery + '" placeholder="Buscar..." class="bg-transparent outline-none font-bold text-xs w-32"></div><button id="btnNewIdea" class="btn-swiss-primary shadow-md">+ NUEVO</button><button onclick="location.reload()" class="btn-swiss-outline py-2 px-4 text-[10px] border-2">SALIR</button></div></header><div class="space-y-3">' + filtered.map(p => { const pr = ((['Idea','Guionizado','Storyboard','Produccion','Finalizado'].indexOf(p.status)+1)/5)*100; let dS = '---'; try { const d = new Date(p.createdAt); if(!isNaN(d)) dS = d.toLocaleDateString(); } catch(e) {} return '<div data-id="' + p.id + '" class="project-row list-row group border-2 bg-white hover:border-brand-primary shadow-sm"><div class="flex-1"><span class="text-[8px] font-bold text-brand-primary uppercase tracking-widest">' + (p.id.startsWith('id-') ? 'NUEVO' : 'REF-'+p.id.substring(0,6)) + '</span><h3 class="text-base font-bold text-brand-dark group-hover:text-brand-primary">' + p.title + '</h3></div><div class="w-32 flex flex-col justify-center shrink-0"><span class="text-[10px] font-bold text-brand-dark truncate">' + (p.team || '---') + '</span><span class="text-[8px] font-bold text-brand-gray uppercase">' + dS + '</span></div><div class="w-40 flex items-center gap-3 shrink-0"><div class="flex-1 h-1.5 bg-brand-hairline rounded-full overflow-hidden border"><div class="h-full bg-brand-primary" style="width: ' + pr + '%"></div></div><span class="text-[9px] font-bold text-brand-dark w-6">' + Math.round(pr) + '%</span></div><div class="w-6 text-right font-bold text-brand-primary opacity-30 group-hover:opacity-100">-></div></div>'; }).join('') + '</div></div>';
        ...
        document.getElementById('ideaForm').onsubmit = async (e) => { e.preventDefault(); const pData = { title: document.getElementById('title').value, description: document.getElementById('description').value }; 
            const payload = { ...pData, status: 'Idea', createdAt: new Date().toISOString(), lastEditor: window.appState.userName || 'Anonimo' };
            if (isSheets) { await fetch(sheetsUrl, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ action: 'save', ...payload, id: 'id-' + Date.now() }) }); setTimeout(loadData, 1500); }
            else { const c = JSON.parse(localStorage.getItem('av_planner_projects') || '[]'); c.push({ ...payload, id: 'local-' + Date.now() }); localStorage.setItem('av_planner_projects', JSON.stringify(c)); loadData(); }
